...
class TestConnectionLoading(unittest.TestCase):
    ...
    def test_load_connections(self):
        from azure.cli.command_modules.cognitiveservices.custom import _load_connection_from_file
        for filename in os.listdir(self.INPUT_DATA_PATH):
            if filename.endswith('.yaml'):
                conn = _load_connection_from_file(os.path.join(self.INPUT_DATA_PATH, filename))
